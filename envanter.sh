...
disk_alani_goster() {
  local usage=$(du -sh . 2>/dev/null)
  zenity_with_size --info --title="Disk Kullanımı" --text="Proje dizininin toplam boyutu:\n$usage"
}

diske_yedek_alma() {
  local backup_dir="backup"
  mkdir -p "$backup_dir"  # Yedekleme klasörünü oluştur
  cp *.csv "$backup_dir"  # Tüm CSV dosyalarını kopyala
  zenity_with_size --info --title="Yedekleme" --text="Tüm CSV dosyaları '$backup_dir' dizinine yedeklendi."
}


hata_kayitlarini_goster() {
  if [ ! -s "$LOG_FILE" ]; then
    zenity_with_size --warning --title="Hata Kayıtları" --text="Hata kaydı bulunmamaktadır."
    return
  fi
  zenity_with_size --text-info --title="Hata Kayıtları" --filename="$LOG_FILE"
}
...
main() {
  while true; do
    local choice=$(zenity_with_size --list --title="Envanter Yönetim Sistemi" --text="Bir işlem seçin:" \
      ...
      "Kullanıcı Ekle" "Kullanıcı Listele" "Kullanıcı Sil" "Bloklu Kullanıcıları Listele" "Kullanıcı Bloğunu Kaldır" "Disk Kullanımı"  "Yedekleme" "Hata Kayıtları" "Çıkış")
